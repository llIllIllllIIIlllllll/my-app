...
<Select labelText="플레이어1" bind:selected={player1}>
  ...
  <SelectItem value={mens.player}.{mens.univ} text={mens.player} />
  {/each}
</Select>
...
<StructuredList condensed>
  ...
  <StructuredListBody >
    ...
    <StructuredListRow >
      ...
      <StructuredListCell >{match.player1} <img src={match.player1univimg} alt="media"/> <span class="win"></span>  </StructuredListCell>
      <StructuredListCell >{match.map}</StructuredListCell>
      <StructuredListCell ><img src={match.player2univimg} alt="media"/>  {match.player2} </StructuredListCell>
      {:else}
      <StructuredListCell >{match.player1} <img src={match.player1univimg} alt="media"/></StructuredListCell>
      <StructuredListCell >{match.map}</StructuredListCell>
      <StructuredListCell > <span class="win"></span> <img src={match.player2univimg} alt="media"/> {match.player2} </StructuredListCell>
      {/if}
    </StructuredListRow>
    {/each}
...
const sub = authStore.subscribe(async (info) => {
  ...
  onDestroy(() => {
    ...
    if (player1 !== "" && player2 !== "" && w !== "" && map !=="") {
     try {
      ...
      let player1univimg = getUniv(player1univ);
      player2univ =player2.split('.')[1];
      let player2univimg = getUniv(player2univ);
      w = w.split('.')[0];
  ...
  let createPlayer= async ()=>{
    ...
    case '캄성여대':
          univ ="캄성여대";
         univimg="https://firebasestorage.googleapis.com/v0/b/blog-61ad4.appspot.com/o/cs.png?alt=media&token=84c16f35-679d-4ff0-bed5-786a41c15b1a";
         break;
    case '파이스트':
          univ ="파이스트";
